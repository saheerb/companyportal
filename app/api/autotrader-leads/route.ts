...
export async function GET(req: NextRequest) {
  ...
  if (search) {
    conditions.push(`(car_name ILIKE $${idx} OR location ILIKE $${idx} OR phone ILIKE $${idx})`);
    values.push(`%${search}%`);
    idx++;
  }
  if (status) {
  ...
  const { rows } = await pool.query(
    `SELECT * FROM car_leads ${where} ORDER BY scraped_at DESC`,
    values
  );
...
export async function PATCH(req: NextRequest) {
  ...
  if (log_add || log_edit || log_delete) {
    const { rows } = await pool.query(`SELECT activity_log FROM car_leads WHERE id = $1`, [id]);
    ...
    const { rows: updated } = await pool.query(
      `UPDATE car_leads SET activity_log = $1 WHERE id = $2 RETURNING *`,
  ...
  vals.push(id);
  const { rows } = await pool.query(
    `UPDATE car_leads SET ${sets.join(", ")} WHERE id = $${idx} RETURNING *`,
    vals
  );
